...
## 6. 상담 로그 생성 (`internal_generator.py`)
...
## 7. 학생 프로필 생성 (`internal_generator.py`)
...
## 8. 웹/앱 로그 생성 (`internal_generator.py`)
...
## 9. 자격증 시험 일정 생성 (`events_generator.py`)
...
## 11. 계절성 이벤트 생성 (`events_generator.py`)
...
## 14. 학습 파이프라인: CSV → training_dataset.csv

합성 CSV 9종에서 모델 학습용 데이터셋까지의 전처리 파이프라인을 설명한다.
...
# 2단계: training_dataset.csv 생성
python -c "from edupulse.preprocessing.merger import build_training_dataset; build_training_dataset()"
...
**1단계: 병합 (`merger.py → merge_datasets()`)**

enrollment_history.csv를 기준으로 8개 CSV를 `[field, date]` LEFT JOIN:
...
| web_logs.csv | field, date | `page_views`, `cart_abandon_rate` |
...
**2단계: 정제 (`cleaner.py → clean_data()`)**

- 날짜 컬럼 YYYY-MM-DD 표준화
- 수치 결측치 → `interpolate(method='linear')`
- `enrollment_count` IQR 이상치 클리핑

**3단계: 피처 변환 (`transformer.py → add_lag_features()`)**

| 생성 피처 | 로직 |
|----------|------|
| `lag_1w`, `lag_2w`, `lag_4w`, `lag_8w` | `groupby("field").shift(n)` — 분야별 enrollment n주 전 값 |
| `rolling_mean_4w` | `groupby("field").rolling(4).mean()` — 4주 이동평균 |
| `month_sin`, `month_cos` | `sin(2π × month / 12)`, `cos(2π × month / 12)` — 순환 인코딩 |
| `field_encoded` | constants.py `FIELD_ENCODING` (art=0, coding=1, game=2, security=3) |
| `age_group_diversity` | `-Σ(ratio × log(ratio))` — 연령대 3개 비율의 Shannon 엔트로피 |

### 14.3 FEATURE_COLUMNS 매핑 (22개)

최종 `training_dataset.csv`의 34개 컬럼 중, 모델이 학습에 사용하는 22개 피처 (`xgboost_model.py:FEATURE_COLUMNS`):
...
| `conversion_rate` | consultation_logs.csv | merger | 상담→등록 전환율 |
| `page_views` | web_logs.csv | merger | 주간 페이지뷰 |
| `cart_abandon_rate` | web_logs.csv | merger | 장바구니 이탈률 |
| `age_group_diversity` | student_profiles.csv | transformer | 연령대 Shannon 엔트로피 |
...
training_dataset.csv에 포함되지만 모델 학습에 사용되지 않는 컬럼:
...
| `ds`, `y` | Prophet 호환 컬럼 |
| `age_20s_ratio`, `age_30s_ratio`, `age_40plus_ratio` | `age_group_diversity` 파생 원본 |
| `purpose_career`, `purpose_hobby`, `purpose_cert` | 목적 분포 (참고용) |

---

## 15. 합성 상관 관계 인플레이션 경고
...
2024-01-08,coding,85,0.62,2024-01-08,85
```
...
3. training_dataset.csv 재생성: `python -c "from edupulse.preprocessing.merger import build_training_dataset; build_training_dataset()"`
4. 모델 재학습: `python -m edupulse.model.train --model all --version 1`
...
└── training_dataset.csv        # 최종 학습 데이터 (1668행, 34컬럼)
```
