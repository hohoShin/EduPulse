...
> **작성일:** 2026-04-10
> **대상 브랜치:** `feat/frontend-enhancement-v2`
> **관련 문서:** `docs/ai_plans/edupulse-frontend-phases.md`, `docs/EduPulse_전체_기능_정리.md`
...
| `hybrid` | Dashboard 3개 → mock, 나머지 9개 → real | Phase C 점진 전환 |
...
| 메서드 | 엔드포인트 | 설명 |
|---|---|---|
| `getSystemStatus()` | `GET /api/v1/health` | 서버/모델/DB/메모리 상태 |
| `getLeadConversion({field})` | `POST /api/v1/marketing/lead-conversion` | 잠재 수강생 전환 분석 |
| `getDemographics({field})` | `POST /api/v1/simulation/demographics` | 연령/성별 분포 |
| `getCompetitors({field})` | `POST /api/v1/simulation/competitors` | 경쟁 분석 |
| `getClosureRisk({courseName, field, startDate})` | `POST /api/v1/demand/closure-risk` | 폐강 위험 평가 |
...
| `getMarketingTiming({field})` | `marketing/timing` × 3 (HIGH/MID/LOW 병렬) | 수요 등급별 마케팅 타이밍 |
...
```
① POST /simulation/simulate → baseline/optimistic/pessimistic
② 병렬:
   - POST /marketing/timing (baseline.demand_tier)
   - POST /schedule/suggest (baseline.predicted_enrollment)
③ SimulatorResult 조립 (marketing + operations 필드 추가)
```
...
```
Backend (snake_case) → transformer (snake_case 유지) → Page (snake_case 소비) → ✅
```
...
## 7. LSTM 모델 하이퍼파라미터 튜닝

### 7.1 배경

XGBoost(MAPE 16.2%)와 달리 LSTM은 합성 데이터(1,668행)에서 MAPE 46.7%로 성능이 낮았다.
기존 하이퍼파라미터가 큰 데이터셋 기준으로 설정되어 있어 소규모 데이터에 과적합되는 문제가 있었다.

### 7.2 변경 내용

| 파라미터 | 변경 전 | 변경 후 | 근거 |
|---|---|---|---|
| `SEQUENCE_LENGTH` | 12 | 8 | 1,668행에서 12주 시퀀스는 학습 샘플 부족 |
| `HIDDEN_SIZE` | 64 | 32 | 파라미터 수 축소로 과적합 방지 |
| `DROPOUT` | 0.3 | 0.2 | 작은 모델에서 과도한 드롭아웃은 학습 방해 |
| `PATIENCE` | 7 | 15 | 조기 종료가 너무 빨라 수렴 전 중단됨 |
| `SCHEDULER_PATIENCE` | 3 | 5 | LR 감소 전 충분한 학습 기회 |
| `epochs` | 100 | 200 | 더 긴 학습 기회 제공 |
| `learning_rate` | 1e-3 | 5e-4 | 작은 모델에서 큰 학습률은 발산 위험 |

### 7.3 결과

재학습 결과 MAPE는 40.3%로, 이전(46.7%)보다 개선되었으나 XGBoost(16.2%)에 비해 여전히 높다.
이는 **합성 데이터의 구조적 한계**로 판단된다:
- 1,668행(주간 데이터 8년치)에는 LSTM이 학습할 만한 장기 시퀀스 패턴이 부족
- XGBoost는 개별 행의 피처 조합으로 예측하므로 소규모 데이터에서 유리
- 실제 운영 데이터가 축적되면 LSTM 성능이 더 크게 향상될 것으로 예상

### 7.4 데이터 규모별 하이퍼파라미터 가이드
...
현재 설정은 1,700행 기준으로 최적화되어 있으며, 실제 데이터 축적에 따라 상수 값만 조정하면 된다.

---

## 8. 알려진 제한사항

### 8.1 Prophet 모델 로딩 실패
...
## 10. Frontend ↔ Backend 엔드포인트 매핑 전체표
...
| `getMarketingTiming` | POST | `/api/v1/marketing/timing` × 3 | `course_name`, `start_date`, `demand_tier` |
