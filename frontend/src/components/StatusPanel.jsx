const getIcon = (variant) => {
  switch (variant) {
    ...
    case 'loading':
      return (
        <svg aria-hidden="true" focusable="false" className="animate-spin" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
const StatusPanel = ({ variant = 'info', title, message, children }) => (
  <div className={`state-panel state-panel--${variant}`}>
    <div className="state-panel__row">
      <div className="state-panel__icon">{getIcon(variant)}</div>
      <div className="state-panel__body">
        ...
        {children && <div className="state-panel__content">{children}</div>}
      </div>
    </div>
  </div>
);
